...
void Program::addParameterBlock(double* parameters, int size,
  ...
  parameter_block_ind_.push_back(num_parameters_);
...
void Program::addResidualBlock(const CostFunction*  cost_function,
  ...
  std::vector<int> parameter_block_ids(num_parameter_blocks);
  for (int pi = 0; pi < num_parameter_blocks; ++pi) {
    ...
    parameter_block_ids[pi] = parameter_block_id;
  ...
  ResidualBlock::Ptr new_residual_block =
      ResidualBlock::create(cost_function, loss_function, parameter_block_ids);
...
int Program::numParameterBlockSize() const { return parameter_blocks_.size(); }
